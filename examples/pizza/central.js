    ...
    peripheral.discoverServices([pizzaServiceUuid], (err, services) => {
      services.forEach(service => {
        ...
        service.discoverCharacteristics([], (err, characteristics) => {
          ...
          //
          if (pizzaCrustCharacteristic && pizzaToppingsCharacteristic && pizzaBakeCharacteristic) {
            //
...
function bakePizza() {
  ...
  pizzaCrustCharacteristic.write(crust, false, err => {
    if (!err) {
      ...
      toppings.writeUInt16BE(pizza.PizzaToppings.EXTRA_CHEESE | pizza.PizzaToppings.CANADIAN_BACON | pizza.PizzaToppings.PINEAPPLE, 0);
